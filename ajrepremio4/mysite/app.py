...
app.config['MAIL_USERNAME'] = os.environ.get('MAIL_USERNAME', 'ajrepremio4@gmail.com')  # Puedes sobreescribir en entorno
app.config['MAIL_PASSWORD'] = os.environ.get('MAIL_PASSWORD')  # ⚠️ Define esto en las Environment variables de PythonAnywhere
if not app.config['MAIL_PASSWORD']:
    print('WARNING: MAIL_PASSWORD no definido. Configura MAIL_PASSWORD en Web -> Environment variables en PythonAnywhere')
mail = Mail(app)
...
def iniciar_quiz_grupal(id_sesion):
    ...
    try:
        ...
        cursor.execute('''
            SELECT gs.*, q.id_profesor, q.tiempo_cuenta_regresiva 
            FROM sesiones_juego gs
            JOIN quizzes q ON gs.quiz_id = q.id
            WHERE gs.id = %s
        ''', (id_sesion,))
        ...
        if total and int(total) > 0:
            ...
            tcr = datos_sesion.get('tiempo_cuenta_regresiva', 30) or 30
            duracion_seg = int((cnt or 1) * tcr)
    ...
    try:
        cursor.execute('''
            ...
            GROUP BY 
                ...
                gs.intentos_restantes, gs.intentos_permitidos, q.id, q.titulo, q.modo, q.tiempo_cuenta_regresiva
